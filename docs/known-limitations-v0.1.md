...
- The v0.1.0 repository/package metadata and run manifests report version `0.1.0`.
- `package.json` deliberately remains `private: true` for v0.1.0. This preserves an npm publish guard while still allowing `npm pack`, tarball inspection, clean tarball install, exports/type checks, tests, and demos.
- The v0.1.0 release is distributed as source/GitHub Release plus the live GitHub Pages demo; it is not published to npm.
- If npm publication is chosen later, changing `private` is an explicit action performed only on an exact tested candidate.
